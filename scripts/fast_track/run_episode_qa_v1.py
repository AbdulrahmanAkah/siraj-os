"""Run the local QA stage through the canonical composition root."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = orchestrator.execute(mode="plan" if args.mode == "plan" else "run-stage", stage_id="qa_gate")
    payload = {"status": result.get("status", result.get("plan", {}).get("requested_mode", "PLAN")), "result": result, "network": False}
