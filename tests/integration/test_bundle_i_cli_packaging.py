...
def test_cli_v2_stable_json_and_exit_codes(capsys):
 ...
 payload=json.loads(execute("health",True));assert payload["status"]=="SUCCESS" and payload["version"]=="0.1.0-rc.1"
...
def test_release_candidate_package_metadata_and_docs_exist():
 ...
 assert "version = \"0.1.0rc1\"" in (root/"pyproject.toml").read_text()
