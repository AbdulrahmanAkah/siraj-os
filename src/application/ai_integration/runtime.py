...
class AIIntegrationGateway:
 def execute(self,provider,prompt,evidence_ids,request_text,known_claim_ids=None,contradicted_claim_ids=None):
  if not prompt.version or not prompt.template:raise ValueError("Invalid prompt contract")
  evidence=stable_unique(evidence_ids);context=EvidenceContext(deterministic_id("evidence_context",evidence),evidence,[],integrity_hash(evidence));response=provider.generate({"prompt_id":prompt.prompt_id,"text":request_text,"evidence_ids":evidence});citations=stable_unique(response.get("citations",[]));claims=stable_unique(response.get("claims",[]));trace=AITrace(deterministic_id("ai_trace",[evidence,citations]),evidence_ids=evidence,citation_ids=citations,claim_ids=claims);generation=GroundedGenerationResult(deterministic_id("grounded_generation",[response["text"],citations]),response["text"],citations,claims,trace)
  issues=[]
  ...
  if known_claim_ids is not None and any(x not in set(known_claim_ids) for x in claims):issues.append(AIValidationIssue(deterministic_id("ai_issue",claims),"UNSUPPORTED_CLAIM",generation.result_id,"ERROR"))
  if contradicted_claim_ids is not None and any(x in set(contradicted_claim_ids) for x in claims):issues.append(AIValidationIssue(deterministic_id("ai_issue",claims),"CONTRADICTED_CRITICAL_CLAIM",generation.result_id,"CRITICAL"))
  injection_markers=("ignore previous","system prompt","reveal credential","fabricate citation","disable validation")
  if any(marker in request_text.lower() for marker in injection_markers):issues.append(AIValidationIssue(deterministic_id("ai_issue","PROMPT_INJECTION_SIGNAL"),"PROMPT_INJECTION_SIGNAL",generation.result_id,"ERROR"))
